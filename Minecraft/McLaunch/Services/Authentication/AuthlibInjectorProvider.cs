namespace PCL.Core.Minecraft.McLaunch.Services.Authentication;
...
public class AuthlibInjectorProvider : AuthenticationProviderBase {
    ...
    public override async Task<Result<McLoginResult>> AuthenticateAsync() {
        try {
            ...
            return Result<McLoginResult>.Failed($"第三方验证失败: {ex.Message}", ex);
        }
    }

    protected override async Task<Result<McLoginResult>> DoValidateAsync(string accessToken, string clientToken) {
        try {
            LogAuth("验证第三方令牌");
            ...
            return Result<McLoginResult>.Success(new McLoginResult {
            ...
            return Result<McLoginResult>.Failed($"令牌验证失败: {ex.Message}", ex);
        }
    }

    protected override async Task<Result<McLoginResult>> DoRefreshAsync(string refreshToken) {
        try {
            LogAuth("刷新第三方令牌");
            ...
            if (loginJson["selectedProfile"] == null)
                return Result<McLoginResult>.Failed("刷新后未找到选定的角色");

            var result = new McLoginResult {
            ...
            LogAuth("令牌刷新成功");
            return Result<McLoginResult>.Success(result);
        } catch (Exception ex) {
            LogAuth($"令牌刷新失败: {ex.Message}");
            return Result<McLoginResult>.Failed($"令牌刷新失败: {ex.Message}", ex);
        }
    }

    private async Task<Result<McLoginResult>> LoginAsync() {
        try {
            var requestData = new JObject(
            ...
            if (loginJson["availableProfiles"]?.Count() == 0)
                return Result<McLoginResult>.Failed("该账户没有创建角色，请先创建角色后再试！");

            // 选择角色
            var (selectedName, selectedId) = await SelectProfileAsync(loginJson);
            if (string.IsNullOrEmpty(selectedName))
                return Result<McLoginResult>.Failed("未选择有效的角色");
            ...
            return Result<McLoginResult>.Failed($"登录请求失败: {ex.Message}", ex);
        }
    }
