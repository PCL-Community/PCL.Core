...
public sealed class SourceController
{
        ...
        try
        {
            if (_currentSource != null)
            {
                try
                {
                    ...
                    LogWrapper.Info("Update", "使用默认源处理成功");
                    return res;
                }
                catch
                {
                    LogWrapper.Warn("Update", "默认源失效，遍历其他更新源");
                }
            }
            else
            {
                LogWrapper.Info("Update", "无默认源，遍历其他更新源");
            }
            ...
            foreach (var source in _availableSources)
            {
                try
                {
                    ...
                    LogWrapper.Info("Update", $"使用 {source.SourceName} 处理成功");
                    return res;
                }
                catch
                {
                    LogWrapper.Warn("Update", $"{source.SourceName}失效，使用下一个更新源");
                }
            }
    ...
    {
        var data = await _TryFindSourceAsync(s => s.CheckUpdateAsync());
    ...
    public async Task<VersionAnnouncementDataModel> GetAnnouncementListAsync() => 
        await _TryFindSourceAsync(s => s.GetAnnouncementAsync());

    /// <summary>
    /// 使用可用源下载到指定路径。
    /// </summary>
    public async Task DownloadAsync(string outputPath) => 
        await _TryFindSourceAsync(s => s.DownloadAsync(outputPath));
}
