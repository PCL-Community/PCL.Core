...
public class LaunchArgumentBuilder {
    private readonly McInstance.McInstance _instance;
    private readonly Java _selectedJava;
    private readonly McLoginResult _loginResult;
        
    public LaunchArgumentBuilder(McInstance instance, Java selectedJava, McLoginResult loginResult)
    {
        _instance = instance;
        _selectedJava = selectedJava;
    ...
    private void McLaunchArgumentMain(string a) {
        if (McInstanceManager.McInstanceCurrent == null) return;
        
        McLaunchUtils.Log("开始获取 Minecraft 启动参数");
        // 获取基准字符串与参数信息
        var arguments = "";
        if (McInstanceManager.McInstanceCurrent._versionJson!.TryGetPropertyValue("arguments", out var argumentNode) &&
            ...
            arguments = McLaunchArgumentsJvmNew(McInstanceManager.McInstanceCurrent);
            McLaunchUtils.Log("新版 JVM 参数获取成功：");
            McLaunchUtils.Log(arguments);
        } else {
            McLaunchUtils.Log("获取旧版 JVM 参数");
            arguments = McLaunchArgumentsJvmOld(McInstanceManager.McInstanceCurrent);
            McLaunchUtils.Log("旧版 JVM 参数获取成功：");
            McLaunchUtils.Log(arguments);
        }

        if (!string.IsNullOrEmpty(McInstanceManager.McInstanceCurrent._versionJson["minecraftArguments"]?.ToString())) {
            McLaunchUtils.Log("获取旧版 Game 参数");
            arguments += " " + McLaunchArgumentsGameOld(McInstanceManager.McInstanceCurrent);
            McLaunchUtils.Log("旧版 Game 参数获取成功");
        }

        if (McInstanceManager.McInstanceCurrent._versionJson!.TryGetPropertyValue("arguments", out var argumentNode2) &&
            ...
            arguments += " " + McLaunchArgumentsGameNew(McInstanceManager.McInstanceCurrent);
        ...
        // 自定义参数
        var argumentGame = Config.Instance.GameArgs[McInstanceManager.McInstanceCurrent.Path];
        arguments += " " + (string.IsNullOrEmpty(argumentGame) ? Setup.Get("LaunchAdvanceGame") : argumentGame);

        // 替换参数
        var replaceArguments = McLaunchArgumentsReplace(McInstanceManager.McInstanceCurrent, loader);
        ...
        string server = string.IsNullOrEmpty(CurrentLaunchOptions.ServerIp)
            ? Config.Instance.ServerToEnter[McInstanceManager.McInstanceCurrent.Path]
            : CurrentLaunchOptions.ServerIp;
        if (string.IsNullOrWhiteSpace(worldName) && !string.IsNullOrWhiteSpace(server)) {
            if (McInstanceManager.McInstanceCurrent.ReleaseTime > new DateTime(2023, 4, 4)) {
                ...
                if (McInstanceManager.McInstanceCurrent.HasPatcher("OptiFine")) {
