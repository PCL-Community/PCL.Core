...
public static class PreCheckService {
    ...
    private static void ValidatePaths(CancellationTokenSource source) {
        if (InstanceManager.Current == null) {
            throw new InvalidOperationException("未选择Minecraft实例");
        }

        // 检查路径中的特殊字符
        if (InstanceManager.Current.IsolatedPath.Contains('!') || InstanceManager.Current.IsolatedPath.Contains(';')) {
            throw new InvalidOperationException($"游戏路径中不可包含 ! 或 ;（{InstanceManager.Current.IsolatedPath}）");
        }

        // UTF-8代码页下的路径检查
        if (EncodingUtils.IsDefaultEncodingUtf8() && !Config.Hint.NonAsciiGamePath && !InstanceManager.Current.Path.IsASCII()) {
            var userChoice = MsgBoxWrapper.Show(
                $"欲启动实例 \"{InstanceManager.Current.Name}\" 的路径中存在可能影响游戏正常运行的字符（非 ASCII 字符），是否仍旧启动游戏？\n\n如果不清楚具体作用，你可以先选择 \"继续\"，发现游戏在启动后很快出现崩溃的情况后再尝试修改游戏路径等操作",
                "游戏路径检查",
                buttons: [
                    "继续",
                    "返回处理",
    ...
    private static void ValidateInstance() {
        try {
            InstanceManager.Current!.Load();
            if (InstanceManager.Current.CardType == McInstanceCardType.Error) {
                throw new InvalidOperationException($"Minecraft存在问题：{InstanceManager.Current.Desc}");
            }
        } catch (Exception ex) {
